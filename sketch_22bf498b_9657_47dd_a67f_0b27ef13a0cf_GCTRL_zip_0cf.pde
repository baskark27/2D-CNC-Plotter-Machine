...
void selectSerialPort() {
  String result = (String) JOptionPane.showInputDialog(
    null, // Corrected from 'frame' to 'null'
...
void draw() {
  ...
  text("s: toggle sleep mode", 12, y); y += dy;
  y = height - dy;
  text("current jog speed: " + speed + " inches per step", 12, y); y -= dy;
  text("current serial port: " + portname, 12, y); y -= dy;
  text("sleep mode: " + (sleepMode ? "ON" : "OFF"), 12, y); y -= dy;
}
...
void keyPressed() {
  ...
  if (!streaming) {
    if (keyCode == LEFT) port.write("G21/G90/G1 X-40  F100\n");
    if (keyCode == RIGHT) port.write("G21/G90/G1 X40 F100\n");
    if (keyCode == UP) port.write("G21/G90/G1 Y40 F100\n");
    if (keyCode == DOWN) port.write("G21/G90/G1 Y-40 F100\n");
